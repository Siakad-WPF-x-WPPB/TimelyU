...
class FrsController extends GetxController {
  ...
  final RxList<JadwalMatakuliahModel> _rawJadwalPilihanList =
      <JadwalMatakuliahModel>[].obs;
  ...
  int? _getStartYearFromTahunAjarString(String? tahunAjarStr) {
    ...
    try {
      ...
      print(
        "FrsController (_getStartYearFromTahunAjarString): Error parsing tahun ajar string '$tahunAjarStr': $e",
      );
    }
    return null;
  }
  ...
  List<JadwalMatakuliahModel> get filteredJadwalPilihanList {
    ...
    print(
      "FrsController (Getter filteredJadwalPilihanList): _rawJadwalPilihanList count = ${_rawJadwalPilihanList.length}",
    );
    print(
      "FrsController (Getter filteredJadwalPilihanList): _idsJadwalFrsDiambilPadaPeriodeIni count = ${_idsJadwalFrsDiambilPadaPeriodeIni.length}, IDs: ${_idsJadwalFrsDiambilPadaPeriodeIni.join(', ')}",
    );

    List<JadwalMatakuliahModel> listSetelahFilterFrs =
        _rawJadwalPilihanList.where((jadwal) {
          return !_idsJadwalFrsDiambilPadaPeriodeIni.contains(jadwal.idJadwal);
        }).toList();
    print(
      "FrsController (Getter filteredJadwalPilihanList): listSetelahFilterFrs (after FRS taken filter) count = ${listSetelahFilterFrs.length}",
    );
    ...
    final result =
        listSetelahFilterFrs.where((jadwal) {
          return jadwal.namaMatakuliah.toLowerCase().contains(query) ||
              jadwal.namaDosen.toLowerCase().contains(query);
        }).toList();
    print(
      "FrsController (Getter filteredJadwalPilihanList): After search query. Result count = ${result.length}",
    );
    return result;
  }
  ...
  void onInit() {
    ...
    debounce(
      searchQuery,
      (_) => _updateJadwalPilihanListForUiConditions(),
  ...
  void _loadInitialData() {
    ...
    if (tahunAjarItems.isNotEmpty && selectedTahunAjar.value == null) {
      ...
      print(
        "FrsController (_loadInitialData): selectedTahunAjar initialized to ${selectedTahunAjar.value}",
      );
    }
    if (semesterItems.isNotEmpty && selectedSemester.value == null) {
      selectedSemester.value = semesterItems.first;
      print(
        "FrsController (_loadInitialData): selectedSemester initialized to ${selectedSemester.value}",
      );
    }
    if (selectedTahunAjar.value != null && selectedSemester.value != null) {
      ...
      print(
        "FrsController (_loadInitialData): Initial period not fully set, data refresh skipped.",
      );
  ...
  Future<void> _refreshData() async {
    print(
      "FrsController (_refreshData): Called for TahunAjar: ${selectedTahunAjar.value}, Semester: ${selectedSemester.value}",
    );
    if (selectedTahunAjar.value != null && selectedSemester.value != null) {
      ...
      print(
        "FrsController (_refreshData): Cleared lists because period not fully selected.",
      );
    }
  }
  ...
  void _clearAllFrsRelatedData() {
    print(
      "FrsController (_clearAllFrsRelatedData): Clearing all FRS related lists.",
    );
    frsList.clear();
  ...
  Future<void> fetchFrsData() async {
    if (selectedTahunAjar.value == null || selectedSemester.value == null) {
      print(
        "FrsController (fetchFrsData): Tahun ajar atau semester null, FRS data fetch skipped.",
      );
      frsList.clear();
      _idsJadwalFrsDiambilPadaPeriodeIni.clear();
    ...
    print(
      "FrsController (fetchFrsData): Fetching FRS data for ${selectedTahunAjar.value} - ${selectedSemester.value}",
    );
    try {
      ...
      if (apiResponse.success && apiResponse.data != null) {
        ...
        print(
          "FrsController (fetchFrsData): frsList updated. Count: ${frsList.length}.",
        );
        if (frsList.isNotEmpty) {
          print(
            "FrsController (fetchFrsData): Sample FRS item - ID Asal: ${frsList.first.idJadwalAsal}, Matkul: ${frsList.first.namaMatakuliah}, Status: ${frsList.first.status}, TA: ${frsList.first.tahunAjar}, Sem: ${frsList.first.semester}",
          );
        }

        final int? currentStartYear = _getStartYearFromTahunAjarString(
          selectedTahunAjar.value,
        );
        final String? currentSemester = selectedSemester.value?.toLowerCase();
        print(
          "FrsController (fetchFrsData): Current period for filtering taken FRS - TA: $currentStartYear, Sem: $currentSemester",
        );
        ...
        if (currentStartYear != null && currentSemester != null) {
              ...
              frsList
                  .where(
                    (frs) =>
                        ...
                        frs.idJadwalAsal != null &&
                        frs.idJadwalAsal!.isNotEmpty &&
          ...
          print(
            "FrsController (fetchFrsData): _idsJadwalFrsDiambilPadaPeriodeIni updated. Count: ${newIds.length}. IDs: ${newIds.join(', ')}",
          );
        } else {
          _idsJadwalFrsDiambilPadaPeriodeIni.clear();
          print(
            "FrsController (fetchFrsData): _idsJadwalFrsDiambilPadaPeriodeIni cleared (currentStartYear or currentSemester is null).",
          );
        }
      } else {
        frsList.clear();
        _idsJadwalFrsDiambilPadaPeriodeIni.clear();
        print(
          "FrsController (fetchFrsData): Failed or no FRS data. Message: ${apiResponse.message}. Cleared frsList and _idsJadwalFrsDiambilPadaPeriodeIni.",
        );
      }
    } catch (e) {
      frsList.clear();
      _idsJadwalFrsDiambilPadaPeriodeIni.clear();
      print(
        "FrsController (fetchFrsData): Exception: $e. Cleared frsList and _idsJadwalFrsDiambilPadaPeriodeIni.",
      );
    } finally {
  ...
  Future<void> fetchJadwalPilihan() async {
    if (selectedTahunAjar.value == null || selectedSemester.value == null) {
      print(
        "FrsController (fetchJadwalPilihan): Tahun ajar atau semester null, jadwal pilihan fetch skipped.",
      );
      _rawJadwalPilihanList.clear();
      _updateJadwalPilihanListForUiConditions();
      return;
    ...
    print(
      "FrsController (fetchJadwalPilihan): Fetching jadwal pilihan for ${selectedTahunAjar.value} - ${selectedSemester.value}",
    );
    try {
      ...
      if (apiResponse.success && apiResponse.data != null) {
        ...
        print(
          "FrsController (fetchJadwalPilihan): _rawJadwalPilihanList updated. Count: ${_rawJadwalPilihanList.length}.",
        );
        if (_rawJadwalPilihanList.isNotEmpty) {
          print(
            "FrsController (fetchJadwalPilihan): Sample Jadwal - ID: ${_rawJadwalPilihanList.first.idJadwal}, Matkul: ${_rawJadwalPilihanList.first.namaMatakuliah}",
          );
        }
      } else {
        _rawJadwalPilihanList.clear();
        print(
          "FrsController (fetchJadwalPilihan): Failed or no jadwal pilihan data. Message: ${apiResponse.message}. Cleared _rawJadwalPilihanList.",
        );
        Get.snackbar(
          'Error Memuat Jadwal',
          apiResponse.message ??
              'Terjadi kesalahan saat memuat jadwal pilihan.',
      ...
      _rawJadwalPilihanList.clear();
      print(
        "FrsController (fetchJadwalPilihan): Exception: $e. Cleared _rawJadwalPilihanList.",
      );
      Get.snackbar(
        'Error Kritis Jadwal',
        'Terjadi pengecualian: ${e.toString()}',
        snackPosition: SnackPosition.BOTTOM,
  ...
  void _updateJadwalPilihanListForUiConditions() {
    ...
    print(
      "FrsController (_updateJadwalPilihanListForUiConditions): jadwalPilihanListForUiConditions updated. Count: ${jadwalPilihanListForUiConditions.length}",
    );
  }
  ...
  void onTahunAjarChanged(String? value) {
    if (value != null && selectedTahunAjar.value != value) {
      print(
        "FrsController (onTahunAjarChanged): Old value = ${selectedTahunAjar.value}, New value = $value",
      );
  ...
  void onSemesterChanged(String? value) {
    if (value != null && selectedSemester.value != value) {
      print(
        "FrsController (onSemesterChanged): Old value = ${selectedSemester.value}, New value = $value",
      );
  ...
  void toggleJadwalSelection(JadwalMatakuliahModel jadwalItem) {
    final isSelected = selectedJadwalUntukPengajuan.any(
      (item) => item.idJadwal == jadwalItem.idJadwal,
    );
    if (isSelected) {
      selectedJadwalUntukPengajuan.removeWhere(
        (item) => item.idJadwal == jadwalItem.idJadwal,
      );
    } else {
  ...
  bool isJadwalSelected(JadwalMatakuliahModel jadwalItem) {
    return selectedJadwalUntukPengajuan.any(
      (item) => item.idJadwal == jadwalItem.idJadwal,
    );
  }
  ...
  Future<void> storeSelectedFrs() async {
    if (selectedJadwalUntukPengajuan.isEmpty) {
      Get.snackbar(
        'Belum Ada Pilihan',
        'Silakan pilih minimal satu mata kuliah untuk diajukan.',
        snackPosition: SnackPosition.BOTTOM,
        backgroundColor: Colors.orange,
        colorText: Colors.white,
      );
    ...
    if (selectedTahunAjar.value == null || selectedSemester.value == null) {
      Get.snackbar(
        ...
        'Pastikan Tahun Ajar dan Semester sudah dipilih.',
        snackPosition: SnackPosition.BOTTOM,
        backgroundColor: Colors.red,
        colorText: Colors.white,
      );
    ...
    Get.defaultDialog(
      ...
      middleText:
          "Anda akan mengajukan ${selectedJadwalUntukPengajuan.length} mata kuliah untuk periode ${selectedTahunAjar.value} Semester ${selectedSemester.value}. Lanjutkan?",
      textConfirm: "Ya, Ajukan",
      textCancel: "Batal",
      confirmTextColor: Colors.white,
      buttonColor:
          Get.theme.primaryColor, // Menggunakan warna primer dari tema GetX
      onConfirm: () async {
  ...
  Future<void> _executeStoreFrs() async {
    ...
    try {
      final List<String> idJadwalDipilih =
          selectedJadwalUntukPengajuan.map((item) => item.idJadwal).toList();
      print(
        "FrsController (_executeStoreFrs): Storing FRS for TA: ${selectedTahunAjar.value}, Sem: ${selectedSemester.value}, Jadwal IDs: ${idJadwalDipilih.join(', ')}",
      );
      ...
      if (apiResponse.success) {
        Get.snackbar(
          'Berhasil',
          apiResponse.message ?? 'FRS berhasil diajukan.',
          snackPosition: SnackPosition.BOTTOM,
          backgroundColor: Colors.green,
          colorText: Colors.white,
        );
        selectedJadwalUntukPengajuan
            .clear(); // Kosongkan pilihan setelah berhasil
        ...
        if (apiResponse.validationErrors != null &&
            apiResponse.validationErrors!.isNotEmpty) {
  ...
  Future<void> deleteFrs(String frsId) async {
    try {
      isLoadingFrs.value = true;

      final token = await _apiService.getToken();
      final headers = _apiService.getHeaders(requiresAuth: true, token: token);

      final response = await http.delete(
        Uri.parse('${BaseApiService.baseUrl}/frs/destroy/$frsId'),
        headers: headers,
      );

      if (response.statusCode == 200) {
        // Hapus item dari list lokal berdasarkan UUID string
        frsList.removeWhere((frs) => frs.id == frsId);

        Get.snackbar(
          'Berhasil',
          'FRS berhasil dihapus',
  ...
  bool isJadwalAlreadyInFrs(JadwalMatakuliahModel jadwal) {
    return frsList.any(
      (frs) =>
          frs.namaMatakuliah == jadwal.namaMatakuliah &&
          frs.kelas == jadwal.kelas &&
          frs.hari == jadwal.hari &&
          frs.jamMulai == jadwal.jamMulai,
    );
  }
}
